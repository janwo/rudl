...
export module SuggestionController {
	...
	export function getPeopleSuggestions(transaction: Transaction, user: User, offset = 0, limit = 25): Promise<User[]> {
		return transaction.run<User, any>("MATCH (u:User) RETURN properties(u) as u LIMIT 5", {
			userId: user.id
		}).then(results => DatabaseManager.neo4jFunctions.unflatten(results.records, 'u'));
	}
	
	export function getRudelSuggestions(transaction: Transaction, user: User, offset = 0, limit = 25): Promise<Rudel[]> {
	...
	export namespace RouteHandlers {
		...
		export function getPeopleSuggestions(request: any, reply: any): void {
			...
			let promise = SuggestionController.getPeopleSuggestions(transaction, request.auth.credentials, request.query.offset, request.query.limit).then((users: User[]) => {
		...
		export function getRudelSuggestions(request: any, reply: any): void {
			let transactionSession = new TransactionSession();
			let transaction = transactionSession.beginTransaction();
			let promise = SuggestionController.getRudelSuggestions(transaction, request.auth.credentials, request.query.offset, request.query.limit).then((rudel: Rudel[]) => {
