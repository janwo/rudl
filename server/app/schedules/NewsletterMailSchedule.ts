...
export const ScheduleConfig = {
	...
	job: () => {
		...
		return session.run(`MATCH(n:Newsletter {ready: true, sent: false}) SET n.sent = true WITH COLLECT(properties(n)) as newsletters MATCH(ns:Settings {newsletterMails: true})<-[:USER_SETTINGS]-(u:User) WHERE u.username = 'maziar' OR u.username = 'jntaylor' WITH COLLECT(properties(u)) as users, newsletters RETURN {users: users, newsletters: newsletters} as result`).then((result: any) => DatabaseManager.neo4jFunctions.unflatten(result.records, 'result').shift()).then((result: {
			users: User[],
            ...
            result.users.forEach(user => {
                result.newsletters.forEach(newsletter => {
                    ...
                    mailJobs.push({
                        ...
                        locale: locale,
                        answerable: newsletter.answerable
                    });
                });
            });
